...
export interface MergeTranscriptsUseCaseInput {
  jobStore: JobStore;
  logger: Logger;
}

export interface MergeTranscriptsUseCaseResult {
  finalMarkdownPath: string;
}

export class MergeTranscriptsUseCase {
  public async execute(input: MergeTranscriptsUseCaseInput): Promise<MergeTranscriptsUseCaseResult> {
