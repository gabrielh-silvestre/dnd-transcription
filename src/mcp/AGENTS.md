...
| `map-params-to-request.ts` | Maps validated tool params -> core `TranscriptionRequest` + derived `chunkDurationMs` (inlined `seconds * 1000` to avoid importing `cli-argument-parser`). |
...
- **o gateway MCP nunca faz `chdir`**: caminhos relativos resolvem contra o `cwd` do processo do servidor (igual ao core), garantindo que o check e o I/O concordem. Nunca adicionar um `process.chdir` aqui.
