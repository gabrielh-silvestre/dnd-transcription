...
| `server.ts` | Builds `McpServer`, registers `transcribe` + `transcription_health`. The `transcribe` handler builds the per-call binding thunk fresh, injects a stderr+buffer logger, runs the MCP output-dir guard, and calls `runTranscriptionCore` in try/catch. |
| `resolve-infra.ts` | R3 startup dispatch: reads `MCP_TRANSCRIPTION_PROVIDER` from env, dispatches to `createOpenAIWhisperConfig` / `createOpenAITranscriptionConfig` / `resolveFakeTranscriberOptions`, lets `ValidationError` abort the process. Holds the env-derived `provider` + binding factory; `createPerCallBindingThunk` builds the per-call thunk. |
...
| `health.ts` | `transcription_health` result: `{ provider, model, backend, ffmpegAvailable, ffprobeAvailable, serverVersion }`. Probes ffmpeg/ffprobe via `runCommand -version`. NEVER returns secrets. |
...
### Internal
`src/core/*` (`runTranscriptionCore`, `DefaultTranscriberBindingFactory`), `src/infrastructure/providers/*` (config factories), `src/shared/*` (logger types, errors, process/`runCommand`, app-version).
