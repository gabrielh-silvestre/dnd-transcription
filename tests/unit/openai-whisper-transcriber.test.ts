...
describe("OpenAI whisper transcriber", () => {
  it("transforma resposta normalizada em TranscriptionResult", async () => {
    ...
    expect(capturedInputs).toStrictEqual([
      {
        audioPath: "/tmp/chunk-0002.wav",
        language: "pt",
        prompt: "glossario",
  ...
  describe("retry policy", () => {
    it("aplica retries limitados para falhas retryaveis", async () => {
      const config = createConfig();
      const delays: number[] = [];
      let attempts = 0;
      const client: OpenAIAudioClient = {
      ...
      expect(result.markdown).toBe("ok apos retry");
      expect(attempts).toBe(3);
      expect(delays).toStrictEqual([15, 15]);
    });

    it("respeita numero maximo de retries", async () => {
      const config = createConfig();
      const delays: number[] = [];
      let attempts = 0;
      const client: OpenAIAudioClient = {
      ...
      await expect(transcriptionPromise).rejects.toThrow(/upstream falhou/);
      expect(attempts).toBe(2);
      expect(delays).toStrictEqual([10]);
    });

    it("nao mascara falha permanente", async () => {
      const config = createConfig();
      const delays: number[] = [];
      let attempts = 0;
      const client: OpenAIAudioClient = {
      ...
      await expect(transcriptionPromise).rejects.toThrow(/request invalido/);
